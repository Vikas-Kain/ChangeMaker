...
const registerUserService = async (userData, userFiles) => {
    try {
        ...
        try {
            const emailDetails = await validateEmail(email.trim())

            if (!emailDetails?.format_valid || !emailDetails?.mx_found) {
                await cleanupFiles(avatarLocalPath, coverImageLocalPath)
                throw new ApiError(400, "Invalid Email")
            }
        } catch (emailError) {
            await cleanupFiles(avatarLocalPath, coverImageLocalPath)
            throw new ApiError(400, "Email validation failed. Please check your email address.")
        }
